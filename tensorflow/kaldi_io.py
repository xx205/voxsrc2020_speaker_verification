...
def read_key(fd):
    ...
    while 1:
        char = fd.read(1)
        c = char.decode("latin1")
        if c == '' : break
        if c == ' ' : break
        key.append(char)
    key = b''.join(key).decode('utf-8')
    key = key.strip()
...
def write_vec_int(file_or_fd, v, key=''):
    ...
    try:
        if key != '' : fd.write((key+' ').encode("utf-8")) # ark-files have keys (utterance-id),
...
def write_post(file_or_fd, p, key=''):
    ...
    try:
        if key != '':
            fd.write((key+' ').encode("utf-8"))  # ark-files have keys (utterance-id),
        fd.write('\0B'.encode())  # we write binary!
        fd.write('\4'.encode())  # int32 type,
